...
<div id="Login">
	...
	<div id="LoginInner">
		...
		<div class="login-input">
			<h2><?php echo $this->BcForm->label($userModel . '.name', 'アカウント名'); ?></h2>
			<?php //エラー発生時にアカウント名の次にある項目にdisplay:noneが渡されてしまうため、エラー発生時の見た目を変えないようにするための生贄?><br style="display:none" />
			<?php echo $this->BcForm->input($userModel . '.name', array('type' => 'text', 'size' => 16, 'tabindex' => 1, 'placeholder'=>'アカウント名')) ?>
		</div>
		<div class="login-input">
			<h2><?php echo $this->BcForm->label($userModel . '.password', 'パスワード') ?></h2>
			<?php //エラー発生時にパスワードの次にある項目にdisplay:noneが渡されてしまうため、エラー発生時の見た目を変えないようにするための生贄?><br style="display:none" />
