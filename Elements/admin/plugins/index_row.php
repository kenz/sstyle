...
<tr<?php echo $class; ?>>
	<td class="row-tools">
		...
		<?php if ($data['Plugin']['update']): ?>
			<?php $this->BcBaser->link('<i class="material-icons" title="アップデート">&#xE8D7;</i>', array('controller' => 'updaters', 'action' => 'plugin', $data['Plugin']['name']), array('title' => 'アップデート', 'class' => 'btn-update')); ?>
		<?php endif ?>
		<?php if ($data['Plugin']['admin_link'] && $data['Plugin']['status'] && !$data['Plugin']['update'] && !$data['Plugin']['old_version']): ?>
			<?php $this->BcBaser->link('<i class="material-icons" title="管理">&#xE30C;</i>', $data['Plugin']['admin_link'], array('title' => '管理')) ?>
		<?php endif; ?>
		<?php if ($data['Plugin']['status']): ?>
			<?php $this->BcBaser->link('<i class="material-icons" title="無効">&#xE15C;</i>', array('action' => 'ajax_delete', $data['Plugin']['name']), array('title' => '無効', 'class' => 'btn-delete')) ?>
		<?php elseif (!$data['Plugin']['status'] && !$data['Plugin']['update'] && !$data['Plugin']['old_version']): ?>
			<?php $this->BcBaser->link('<i class="material-icons" title="インストール">&#xE2C0;</i>', array('action' => 'install', $data['Plugin']['name']), array('title' => 'インストール')) ?>
		<?php endif ?>
		<?php if (!$data['Plugin']['status']): ?>
