...
$this->BcBaser->js(array('admin/outerClick', 'admin/jquery.fixedMenu', 'admin/toolbar'));
$loginUrl = '';
	...
	<div id="ToolbarInner" class="clearfix">
		<div id="ToolMenu">
			<ul>
				...
				<?php elseif (!empty($this->request->params['admin']) || $authPrefix == $currentPrefix || ('/' . $this->request->url) == $loginUrl): ?>
					<li><?php $this->BcBaser->link($this->BcBaser->getImg("logo.svg",array('alt' => 'top','title' => 'top','width'=>'60','height'=>'60')),"/"); ?></li>
				<?php else: ?>
					<?php if ($authPrefix == 'admin'): ?>
						<li><?php $this->BcBaser->link($this->BcBaser->getImg('admin/logo_header.svg', array('width'=>20,'height'=>20,'alt' => 'baserCMS管理システム', 'class' => 'bc-btn')), array('plugin' => null, 'admin' => true, 'controller' => 'dashboard', 'action' => 'index'), array('title' => 'ダッシュボード')) ?></li>
					<?php else: ?>
				...
				<?php if ($this->BcBaser->existsEditLink()): ?>
					<li><?php $this->BcBaser->link('<i title="編集する" class="material-icons">&#xE3C9;</i>', $this->BcBaser->_View->viewVars['editLink'], array('class' => 'tool-menu')); ?></li>
				<?php endif ?>
				<?php if ($this->BcBaser->existsPublishLink()): ?>
					<li><?php $this->BcBaser->link('<i title="公開ページ" class="material-icons">&#xE80B;</i>', $this->BcBaser->_View->viewVars['publishLink'], array('class' => 'tool-menu')); ?> </li>
				<?php endif ?>
				<?php if (!$loginUrl || $this->request->url != $loginUrl): ?>
					<?php if (Configure::read('debug') == -1 && $this->name != "Installations"): ?>
						<li>&nbsp;&nbsp;<span id="DebugMode" title="インストールモードです。運営を開始する前にシステム設定よりノーマルモードに戻しましょう。"><i class="material-icons">&#xE85A;</i></span>&nbsp;&nbsp;</li>
					<?php elseif (Configure::read('debug') > 0): ?>
						<li>&nbsp;&nbsp;<span id="DebugMode" title="デバッグモード<?php echo mb_convert_kana(Configure::read('debug'), 'N') ?>です。運営を開始する前にシステム設定よりノーマルモードに戻しましょう。"><i class="material-icons">&#xE85A;</i></span>&nbsp;&nbsp;</li>
					<?php endif; ?>
				<?php endif ?>
			</ul>
		</div>
		<div id="UserMenu">
			<ul class="clearfix">
				<li>
					<?php if (!empty($user)): ?>
						<?php $this->BcBaser->link('<i title='.$this->BcBaser->getUserName($user).' class="material-icons">&#xE7FD;</i>' . ' ' . $this->BcBaser->getImg('admin/btn_dropdown.png', array('width' => 8, 'height' => 11, 'class' => 'bc-btn')), 'javascript:void(0)', array('class' => 'title')) ?>
						<ul>
				...
				<?php if (!empty($user) && $authPrefix == 'admin'): ?>
					<li>
						<?php $this->BcBaser->link('<i class="material-icons">&#xE871;</i>' . ' ' . $this->BcBaser->getImg('admin/btn_dropdown.png', array('width' => 8, 'height' => 11, 'class' => 'bc-btn')), 'javascript:void(0)', array('class' => 'title')) ?>
